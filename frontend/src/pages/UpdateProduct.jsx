...
function UpdateProduct() {
  ...
  const handleChange = (e) => {
    ...
    const numericValue =
      ...
      name === "displayed_price"
        ? value === ""
          ? ""
          : Number(value) // Check if value is empty, if so keep it empty
        : value;
  ...
  const handleSubmit = (e) => {
    ...
    if (inputs.quantity !== "" && inputs.quantity < 0)
      newErrors.quantity = "Quantity cannot be negative"; // Only validate if not empty
    if (inputs.price !== "" && inputs.price < 0)
      newErrors.price = "Price cannot be negative"; // Only validate if not empty
  ...
  const sendRequest = async () => {
    try {
      await api.put(`/Products/products-update/${id}`, {
