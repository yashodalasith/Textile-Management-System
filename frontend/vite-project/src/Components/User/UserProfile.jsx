...
function UserProfile() {
    ...
    return (
                ...
                <div className="prof-form-container">
                    <form>
                        <h2>Welcome Back😊 <h3 className='user-name'>{user ? user.name : ""}</h3></h2>
                        <div className='circle'>{user ? user.name.charAt(0).toUpperCase(): ""}</div>
                        <div className='user-details'>
                            <table>
                                <tbody>
                                    <tr><td>Name:</td><td>{user ? user.name : ""}</td></tr>
                                    <tr><td>Email:</td><td>{user ? user.email : ""}</td></tr>
                                    <tr><td>Phone:</td><td>{user ? user.phone : ""}</td></tr>
                                    <tr><td>Address:</td><td>{user ? user.address :   ""}</td></tr>
                                </tbody>
                            </table>
                        </div>
                        <div className='actions'>
                            <button type="button" className="btn update">Update</button>
                            <button type="button" className="btn delete" onClick={deleteHandler}>Delete</button>
                        </div>
